...
```
/plugin marketplace add trigrman/ee-claude-plugins
```
...
| --- | --- | --- |
| `ee-pm` | Mike Mitchell's Equal Experts product management toolkit — skills and agents for AI-assisted, human-led product work across discovery and delivery (interviews, synthesis, opportunity trees, assumption maps, story maps, stories, backlog, prototyping, workshop facilitation). | [trigrman/ee-pm](https://github.com/trigrman/ee-pm) |
| `ee-context-mesh` | Structure for where LLM context lives across an organization, plus the process for populating it from conversations — an AI Hub, a locked node/edge vocabulary, and skills to ingest transcripts into staged, human-gated context. | [trigrman/context-mesh-claude](https://github.com/trigrman/context-mesh-claude) |
...
2. Add an entry to the `plugins` array in `.claude-plugin/marketplace.json`, pointing at that repo:
   ...
   {
     "name": "my-plugin",
     "source": { "source": "github", "repo": "trigrman/my-plugin-repo" },
